...
// 가용 블록들을 이전 가용 블록과 다음 가용 블록을 가리키는 포인터를 가진 
// 일종의 이중 연결 링크드 리스트 자료구조로 구성

// pred(predeccesor)와 succ(successor) 포인터를 포함하는
// 이중 연결 리스트로 구성

// 할당된 블록의 구조는 묵시적 리스트 방식과 동일
// 가용 블록의 구조만 바뀌는 것이다.

// first fit 할당 시간을 전체 블록 수에 비례하는 것에서
// 가용 블록의 수에 비례하는 것으로 줄인다.
// 반환 시에는 가용 리스트 내에서 블록 정렬하는 방식에 따리
// 블록 수에 비례하거나 상수 시간을 가질 수 있다.

// lifo순서와 first-fit 배치 정책을 사용 시
// 할당기는 대부분 최근에 사용된 블록들을 먼저 조사
/*
...
static void *find_fit(size_t asize){
    ...
    for (bp = SUCC_P(heap_listp); !GET_SIZE(HDRP(bp)); bp = SUCC_P(bp)){
